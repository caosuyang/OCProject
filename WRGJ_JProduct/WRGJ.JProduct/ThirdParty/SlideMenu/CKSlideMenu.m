...
- (void)loadBodyContentAtIndex:(NSInteger)index
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(menuSelectIndex:)]) {
        [self.delegate menuSelectIndex:index];
    }
...
- (void)itemClicked:(UIButton *)button
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(menuSelectIndex:)]) {
        [self.delegate menuSelectIndex:_currentIndex];
    }
}
...
- (void)scrollToIndex:(NSInteger)toIndex
{
    ...
    void (^completeAction)(void) = ^(){
        [self->_bodyScrollView setContentOffset:CGPointMake(self->_bodyScrollView.frame.size.width*toIndex, 0) animated:NO];
        [self resetTabScrollViewFrame];
    };
